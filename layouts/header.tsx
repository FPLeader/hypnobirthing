...
export default function Header() {
    ...
    const style = {
        ...
        dropDownInner: 'absolute top-10 overflow-hidden h-0 group-hover:h-[450px] opacity-0 group-hover:opacity-100 transactioin-all duration-500',
                        ...
                        <div className={style.dropDownInner}>
                            <div className={style.dropDownItems}>
                                {LinkPart1.slice(1).map((obj: LinkType, index: number) => (
                                    <div key={index} className={style.dropDownItem} onClick={() => router.push(obj.link)}>
                                        {obj.name}
                                    </div>
                                ))}
                            </div>
